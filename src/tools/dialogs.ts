...
const audienceInput = {
  ...
  language: z
    ...
    .max(12)
    .optional()
    .describe('Язык собеседника, как его сообщает Telegram: ru, en.'),
...
export const dialogTools: Tool[] = [
  ...
  tool({
    ...
    input: {
      case: caseField,
      dialog: z.string().describe('Диалог: имя собеседника, @username, номер чата или идентификатор.'),
      text: z.string().min(1).max(4096).optional().describe('Текст сообщения.'),
      reply: z
        .string()
        .optional()
        .describe(
  ...
  tool({
    ...
    input: {
      ...
      text: z.string().min(1).max(4096).optional().describe('Текст ответа.'),
    },
    async run(args, ctx) {
      const found = await ctx.resolveCase(args.case);
  ...
  tool({
    ...
    input: {
      ...
      parse_mode: z
        .enum(['', 'HTML'])
        .optional()
        .describe('HTML — разметка Telegram. Пусто — обычный текст. По умолчанию пусто.'),
      ...audienceInput,
    },
    async run(args, ctx) {
      const found = await ctx.resolveCase(args.case);
  ...
  tool({
    ...
    description:
      ...
      'проверяет здесь же: Telegram отбил бы сообщение с незакрытым тегом сразу у всех.',
    input: {
      case: caseField,
      broadcast: z
        .string()
      ...
      parse_mode: z
        .enum(['', 'HTML'])
        .optional()
        .describe('HTML — разметка Telegram. Пусто — обычный текст.'),
      buttons: z
        .array(
          z
            .array(
